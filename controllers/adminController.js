...
const dashboardLoad = async (req, res) => {
  try {
    ...
    const totalSalesCount =
      totalSalesResult.length > 0 ? totalSalesResult[0].totalSalesCount : 0;
    ...
    const orderAmount =
      overallOrderAmount.length > 0 ? overallOrderAmount[0].totalAmount : 0;

    let discount = await Order.aggregate([
      { $match: { status: "Delivered" } },
      { $group: { _id: null, totalDiscount: { $sum: "$discount" } } },
    ]);

    const couponDiscount = await Order.aggregate([
      { $match: { status: "Delivered" } },
      {
    ...
    res.render("dashboard", {
      ...
      couponDiscount: couponDiscount[0]
        ? couponDiscount[0].totalCouponDiscount
        : 0,
      topProducts,
    });
  } catch (error) {
